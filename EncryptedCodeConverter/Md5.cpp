...
int Md5Converter::Encode(const char* pchCode, int size, char* pszBuffer, int bufferSize)
{
    string strCode(pchCode, size);
    string strBuffer = getMD5(strCode);
    if (strBuffer.length() < bufferSize)
    {
        strcpy_s(pszBuffer, bufferSize, strBuffer.c_str());
...
std::string Md5Converter::Encode(string& csCode)
{
    return getMD5(csCode);
}
